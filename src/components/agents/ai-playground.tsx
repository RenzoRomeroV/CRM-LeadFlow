    ...
    <div className="flex h-[60vh] min-h-[420px] flex-col rounded-xl border border-border bg-card">
      ...
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-foreground">Zona de pruebas</span>
          <span className="text-xs text-muted-foreground">
            — Responde a las pruebas como si fueras un cliente.
          </span>
        </div>
        <Button
          variant="ghost"
        ...
        >
          <RotateCcw className="mr-1.5 h-3.5 w-3.5" /> Resetear
        </Button>
      </div>
        ...
        {turns.length === 0 && (
          <div className="flex h-full flex-col items-center justify-center text-center text-sm text-muted-foreground">
            ...
            <p className="mt-1 text-xs">
              Utiliza tu base de conocimientos y se comporta exactamente igual que el
              bot de respuesta automática, incluyendo la transferencia.
            </p>
            {onGoToSetup && (
              <Button
                variant="link"
              ...
              >
                ¿Aún no está configurado? Vaya a Configuración <ArrowRight className="ml-1 h-3 w-3" />
              </Button>
            )}
          </div>
        )}
